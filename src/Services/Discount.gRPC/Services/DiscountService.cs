...
namespace Discount.gRPC.Services
{
    public class DiscountService : DiscountProtoService.DiscountProtoServiceBase
    {
        ...
        public override async Task<CouponRequest> CreateDiscount(CouponRequest request, ServerCallContext context)
        {
            ...
            if (isSaved)
            {
                _logger.LogInformation("Discount created successfully. ProductName: {productName}", coupon.ProductName);
            }
            else
            {
                _logger.LogInformation("Discount created failed. ProductName: {productName}", coupon.ProductName);
            }
        ...
        {
            var coupon = _mapper.Map<Coupon>(request);
            bool isModified = await _couponRepository.CreateDiscount(coupon);
            if (isModified)
            {
        ...
        public override async Task<DeleteDiscountResponse> DeleteDiscount(DeleteDiscountRequest request, ServerCallContext context)
        {
            ...
            if (isDeleted)
            {
                _logger.LogInformation("Discount deleted successfully. ProductName: {productId}", request.ProductId);
            }
            else
            {
